...
//Desenvolvedora com os atributos, nome, data criada e a lista de jogos cadastradas

// Rota para obter todos os desenvolvedores
app.get("/desenvolvedores", (req, res) => {
    const sql = "SELECT * FROM desenvolvedores";
    db.query(sql, (err, results) => {
...
app.post("/desenvolvedores", (req, res) => {
    const { nome, data_criada } = req.body;
    const sql = "INSERT INTO desenvolvedores (nome, data_criada) VALUES (?, ?)";
    db.query(sql, [nome, data_criada], (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(results);
        });
    });
...
app.post("/jogos", (req, res) => {
  ...
  db.query(
    sql,
    [titulo, descricao, imagem_url, video_url, genero_id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json({
        id: result.insertId,
